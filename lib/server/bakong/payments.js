import { createAuditLog, createNotification } from "@/lib/business-events";
...
async function expirePayment(payment) {
  ...
  // Fire SYSTEM notification for payment expiry (best-effort)
  try {
    const orderRef = updated.order?.orderNumber || updated.orderId;
    const userId = updated.order?.userId;
    if (userId) {
      await createNotification(prisma, {
        userId,
        type: "SYSTEM",
        title: "Payment Expired",
        message: `Payment for order #${orderRef} has expired. Please try again.`,
        data: { orderId: updated.orderId, paymentId: updated.id, paymentStatus: "EXPIRED", orderNumber: orderRef },
      });
    }
  } catch (notifError) {
    console.error("[payments] EXPIRED notification error:", notifError?.message);
  }
...
async function failPayment(payment, reason, transaction = null) {
  ...
  // Fire SYSTEM notification for payment failure (best-effort)
  try {
    const orderRef = updated.order?.orderNumber || updated.orderId;
    const userId = updated.order?.userId;
    if (userId) {
      await createNotification(prisma, {
        ...
        data: { orderId: updated.orderId, paymentId: updated.id, paymentStatus: "FAILED", orderNumber: orderRef },
      });
    }
  } catch (notifError) {
    console.error("[payments] FAILED notification error:", notifError?.message);
  }
...
async function markPaymentPaid(payment, transaction) {
  return prisma.$transaction(async (tx) => {
    ...
    // Fire PAYMENT_RECEIVED notification (best-effort, outside transaction)
    try {
      const orderRef = updated.order?.orderNumber || updated.orderId;
      const userId = updated.order?.userId;
      const branchId = updated.order?.branchId || updated.branchId;
      ...
      if (userId) {
        await createNotification(prisma, {
          ...
          message: `Payment of $${amount.toFixed(2)} for order #${orderRef} has been verified.`,
          data: { orderId: updated.orderId, paymentId: updated.id, paymentStatus: "PAID", orderNumber: orderRef },
        });
      }
      ...
      if (branchId) {
        await createNotification(prisma, {
          ...
          message: `KHQR payment of $${amount.toFixed(2)} for order #${orderRef} received.`,
          data: { orderId: updated.orderId, paymentId: updated.id, paymentStatus: "PAID", orderNumber: orderRef },
        });
      }
    } catch (notifError) {
      console.error("[payments] PAYMENT_RECEIVED notification error:", notifError?.message);
    }
