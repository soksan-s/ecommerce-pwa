    ...
    try {
      ...
      if (response.ok || response.status === 409) {
        ...
        if (parsedBody?.id) {
          ...
          if (txn) {
            await put("transactions", { ...txn, synced: true, syncStatus: "synced" });
          }
        }
        ...
        results.push({ id: entry.id, ok: true, status: response.status });
        ...
        results.push({ id: entry.id, ok: false, status: response.status });
      } else {
        await put("sync_logs", {
          url: entry.url,
          status: "failed",
