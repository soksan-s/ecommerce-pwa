...
export function PosLayoutShell({ children }) {
  ...
  const lang = settings?.appearance?.defaultLanguage || settings?.appearance?.language || "en";
  ...
  const localNotifications = usePosStore((state) => state.localNotifications);
  const dismissLocalNotification = usePosStore((state) => state.dismissLocalNotification);
  ...
  async function handleAutoSync() {
    ...
    try {
      ...
      // Emit local notifications for each sync result
        ...
        for (const result of results) {
          ...
          if (result.ok) {
            addLocalNotification({
              type: "success",
              title: "Sale Synced",
              message: receipt ? `Offline sale #${receipt} synchronized successfully.` : "Offline sale synchronized successfully.",
            ...
            addLocalNotification({
              type: "error",
              title: "Sync Failed",
              message: receipt ? `Failed to sync sale #${receipt}.` : `Failed to sync offline sale (HTTP ${result.status || "error"}).`,
              receiptNumber: receipt || null,
            });
          }
        }
  ...
  return (
    <div className="min-h-dvh bg-[var(--background)] text-[var(--foreground)] transition-colors duration-200">
      ...
      >
        <div className={sidebarCollapsed ? "flex flex-col items-center gap-3" : "flex items-start justify-between gap-3"}>
          <div className={sidebarCollapsed ? "text-center" : ""}>
            <p className={sidebarCollapsed ? "text-xs font-black uppercase tracking-[0.08em] text-[var(--pos-action)]" : "text-xs font-bold uppercase tracking-[0.24em] text-[var(--pos-action)]"}>
              {sidebarCollapsed ? "SVS" : (settings?.storeInfo?.storeName || "MyShop")}
            </p>
        ...
        <div className="mt-auto">
          <button
            ...
            className={
              sidebarCollapsed
                ? "flex w-full items-center justify-center rounded-xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/30 p-2.5 text-red-600 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-900/50"
                : "flex w-full items-center gap-2.5 rounded-xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/30 px-3.5 py-2.5 text-xs font-bold text-red-600 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-900/50"
            }
          >
          ...
          <aside className="flex h-full w-72 max-w-[82vw] flex-col bg-[var(--surface-strong)] p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-auto">
              <button
                ...
                className="flex w-full items-center gap-2.5 rounded-xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/30 px-3.5 py-2.5 text-xs font-bold text-red-600 dark:text-red-400 transition hover:bg-red-100"
              >
                <LogOut className="size-4 shrink-0" />
                {t("logout") || "Log Out"}
              </button>
      ...
      <div className={sidebarCollapsed ? "md:pl-20 transition-[padding] duration-200" : "md:pl-64 transition-[padding] duration-200"}>
        {/* Desktop Top Header Bar */}
        <header className="sticky top-0 z-30 hidden h-14 border-b border-[var(--border-soft)] bg-[var(--surface-strong)]/90 px-6 backdrop-blur md:flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            {/* <span className="text-xs font-black uppercase tracking-[0.2em] text-[var(--pos-action)]">
              {settings?.storeInfo?.storeName || "MyShop"}
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            {pendingSyncCount > 0 ? (
              <button
                ...
                className="rounded-full bg-amber-100 dark:bg-amber-950/60 px-2.5 py-0.5 text-xs font-bold text-amber-800 dark:text-amber-300 hover:bg-amber-200 disabled:opacity-50 transition-colors inline-flex items-center gap-1.5"
